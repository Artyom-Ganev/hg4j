...
public class MapTagsToFileRevisions {
    ...
    private static final String REPO = System.getProperty("user.dir") + "/hg4j/src/test/repos/";
...
//		m.revisionMap();
    ...
    public static void main2(String[] args) throws HgCallbackTargetException, HgException, CancelledException, HgRuntimeException {
        final HgRepository repository = new HgLookup().detect(new File(REPO));
        ...
        final Map<Nodeid, List<String>> changeSetRevisionToTags = new HashMap<>();
        ...
        for (String tagName : tagToInfo.keySet()) {
            ...
            List<String> revisionTags = changeSetRevisionToTags.computeIfAbsent(changeSetRevision, k -> new ArrayList<>());
            revisionTags.add(tagName);
        }

        final HgLogCommand logCommand = new HgLogCommand(repository);
        logCommand.file(targetPath, true);
        logCommand.execute((HgChangesetHandler) changeset -> {
    ...
    public void revisionMap() throws Exception {
        ...
        ArrayList<Nodeid> revisions = new ArrayList<>();
        final int step = 5000;
        ...
        HgRevisionMap<HgChangelog> rmap = new HgRevisionMap<>(clog);
    ...
    public void collectTagsPerFile() throws HgException, CancelledException, HgRuntimeException {
        ...
        final HgRevisionMap<HgChangelog> clogrmap = new HgRevisionMap<>(repository.getChangelog()).init();
        // map to look up tag by changeset local number
        final IntMap<List<TagInfo>> tagLocalRev2TagInfo = new IntMap<>(allTags.length);
    ...
    private void buildFile2ChangelogRevisionMap(String... fileNames) throws Exception {
        ...
        HgRevisionMap<HgChangelog> clogMap = new HgRevisionMap<>(clog).init();

        for (String fname : fileNames) {
            HgDataFile fileNode = repository.getFileNode(fname);
            // warm-up
            @SuppressWarnings("UnusedAssignment")
            HgRevisionMap<HgDataFile> fileMap = new HgRevisionMap<>(fileNode).init();
            //
            final int latestRevision = fileNode.getLastRevision();
            //
            final long start_0 = System.nanoTime();
            final Map<Nodeid, Nodeid> changesetToNodeid_0 = new HashMap<>();
            ...
            fileMap = new HgRevisionMap<>(fileNode).init();
            final long start_1a = System.nanoTime();
            final Map<Nodeid, Nodeid> changesetToNodeid_1 = new HashMap<>();
...
//				final Nodeid changesetId = fileNode.getChangesetRevision(nodeId);
            ...
            clogMap = new HgRevisionMap<>(clog).init();
            fileMap = new HgRevisionMap<>(fileNode).init();
            final Map<Nodeid, Nodeid> changesetToNodeid_2 = new HashMap<>();
            ...
            final Map<Nodeid, Nodeid> changesetToNodeid_3 = new HashMap<>();
            fileNode.indexWalk(0, TIP, (HgDataFile.RevisionInspector) (fileRevisionIndex, revision, linkedRevisionIndex) ->
        ...
        for (int i = 0; i < allTags.length; i++) {
            ...
            if (tagRevisionIndex != HgRepository.BAD_REVISION) {
                ...
                if (tagsAssociatedWithRevision == null) {
                    tagLocalRev2TagInfo.put(tagRevisionIndex, tagsAssociatedWithRevision = new LinkedList<>());
                }
    ...
    //
    private void collectTagsPerFile_Approach_1(final HgRevisionMap<HgChangelog> clogrmap, final int[] tagLocalRevs, final TagInfo[] allTags,
            Path targetPath) throws HgException, IllegalArgumentException, HgRuntimeException {
        ...
        final Map<Path, Nodeid[]> file2rev2tag = new HashMap<>();
        ...
        for (int fileRevIndex = 0; fileRevIndex < fileNode.getRevisionCount(); fileRevIndex++) {
            ...
            List<String> associatedTags = new LinkedList<>();
    ...
    private void collectTagsPerFile_Approach_2(HgRepository repository, final int[] tagLocalRevs,
            final IntMap<List<TagInfo>> tagRevIndex2TagInfo, Path targetPath) throws HgException, HgRuntimeException {
        //
        ...
        final Map<Integer, Nodeid> fileRevisionAtTagRevision = new HashMap<>();
        final Map<Nodeid, List<String>> fileRev2TagNames = new HashMap<>();
        HgManifest.Inspector collectFileRevAtCset = new HgManifest.Inspector() {
            ...
            public boolean next(Nodeid nid, Path fname, Flags flags) {
                ...
                if (tagRevIndex2TagInfo.containsKey(csetRevIndex)) {
                    ...
                    if (tags == null) {
                        fileRev2TagNames.put(nid, tags = new ArrayList<>(3));
                    }
        ...
        fileNode.indexWalk(0, TIP, (HgDataFile.RevisionInspector) (fileRevisionIndex, fileRevision, changesetRevisionIndex) -> {
            List<String> associatedTags = new LinkedList<>();
